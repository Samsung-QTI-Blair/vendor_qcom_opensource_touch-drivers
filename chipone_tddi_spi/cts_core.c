...
static void cts_esd_protection_work(struct work_struct *work)
{
    ...
    if(cts_data->cts_dev.rtdata.suspended)//suspend mode
    {
...
static void fts_hardwareinfo_set(void)
{
    ...
    if (strstr(panel_name_tp,"chipone_icnl9951r_wt_dsi_vdo_90hz_boe")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"BOE, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
	}
    if (strstr(panel_name_tp,"hjr_icnl9951r_wt_dsi_vdo_90hz_hkc")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"HJR, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
    if (strstr(panel_name_tp,"txd_icnl9951r_wt_dsi_vdo_90hz_hkc")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"TXD, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
// +P86801AA1 peiyuexiang.wt,add,20230905,compatible XINXIAN
    if (strstr(panel_name_tp,"xinxian_icnl9951r_wt_dsi_vdo_90hz_mdt")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"XINXIAN,%s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
// -P86801AA1 peiyuexiang.wt,add,20230905,compatible XINXIAN
// +P86801AA1 lihesong.wt,add,20230920,compatible dijin
    if (strstr(panel_name_tp,"djn_icnl9951r_wt_dsi_vdo_90hz_boe")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"DIJIN,%s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
// -P86801AA1 lihesong.wt,add,20230920,compatible dijin
// +P86801AA1 zhangjian.wt,add,20241224,compatible xinxian_agc
    if (strstr(panel_name_tp,"xinxian_icnl9951r_wt_dsi_vdo_90hz_mdt_agc")) {
        snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"XINXIAN_AGC,%s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
    hardwareinfo_set_prop(HARDWARE_TP, hardinfo);
// -P86801AA1 zhangjian.wt,add,20241224,compatible xinxian_agc
