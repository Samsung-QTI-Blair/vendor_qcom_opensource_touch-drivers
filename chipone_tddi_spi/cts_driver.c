...
//+P86801AA1,peiyuexiang.wt,modify,20230801,add tp earjack_mode
    if (cts_data->earjack_plug_status) {
        ...
        cts_err("TP_LOG:%s:[tp earjack]EARJACK_PLUG_OUT", __func__);
    }
...
//+P86801AA1,peiyuexiang.wt,modify,20230801,add tp earjack_mode
    if (lux_earphone_mode_status == 1) {
        ...
        cts_err("TP_LOG:%s:[tp earjack]EARJACK_PLUG_IN", __func__);
    }
    if (lux_charger_mode_status == 1) {
        usb_notifier_call_chain_for_tp(USB_PLUG_IN,NULL);
        cts_err("TP_LOG:%s:[tp charger]USB_PLUG_IN", __func__);
    }
//+P86801AA1,peiyuexiang.wt,modify,20230801,add tp earjack_mode
...
#ifdef CFG_CTS_DRM_NOTIFIER
static void cts_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
		 struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger)
			...
			cts_info("normal resume, TP_RESUME_THREAD");
			queue_work(cts_data->workqueue, &cts_data->ts_resume_work);
		}
		break;
	case DRM_PANEL_EVENT_BLANK:
		if (notification->notif_data.early_trigger) {
			cts_info("early suspend, TP_SUSPEND");
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		cts_dbg("shashank:Received fps change old fps:%d new fps:%d",
        ...
        unsigned long action, void *data)
{
    ...
    if (evdata && evdata->data) {
        ...
        } else if (action == FB_EVENT_BLANK) {  //zsw FB_EARLY_EVENT_BLANK
            blank = *(int *)evdata->data;
...
static int cts_init_pm_fb_notifier(struct chipone_ts_data *cts_data)
{
    ...
    if (active_panel) {
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
                    ...
                    &cts_ts_panel_notifier_callback, cts_data);
        if (!cookie)
...
#endif
{
    ...
    int ret = 0;
    if (!panel_name_tp[0]) {
        cts_err("panel_name_tp is missing.");
        return -ENODEV;
	}
//+P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
	if ((strcmp(panel_name_tp,"chipone_icnl9951r_wt_dsi_vdo_90hz_boe") != 0) &&
	(strcmp(panel_name_tp,"hjr_icnl9951r_wt_dsi_vdo_90hz_hkc") != 0) &&
	(strcmp(panel_name_tp,"txd_icnl9951r_wt_dsi_vdo_90hz_hkc") != 0) &&
	(strcmp(panel_name_tp,"xinxian_icnl9951r_wt_dsi_vdo_90hz_mdt") != 0)&&
	(strcmp(panel_name_tp,"djn_icnl9951r_wt_dsi_vdo_90hz_boe") != 0) &&
	(strcmp(panel_name_tp,"xinxian_icnl9951r_wt_dsi_vdo_90hz_mdt_agc") != 0)) {
		cts_err("panel_name_tp Match failed\n");
		return -ENODEV;
    }
//-P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
...
MODULE_PARM_DESC(dsi_display0,
	"icnl9951r.dsi_display0=<display node>: where <display node> is 'primary dsi display node name'");
