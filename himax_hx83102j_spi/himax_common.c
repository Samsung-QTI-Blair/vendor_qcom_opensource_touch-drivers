...
#define HX_PROC_TOUCH_FOLDER            "touchscreen"
#define HX_PROC_SELF_TEST_FILE          "ctp_openshort_test"
// -P86801AA1 peiyuexiang.wt,add,20230703,himax openshort
...
int himax_common_proc_init(void)
{
	...
	proc_self_test_file = proc_create(HX_PROC_SELF_TEST_FILE, 0777,
...
int himax_report_data_init(void)
{
	...
	hx_s_touch_data->rawdata_size_full_stack =
		hx_s_touch_data->touch_all_size - HX_STACK_ORG_LEN;
#if defined(SEC_PALM_FUNC)
	hx_s_touch_data->touch_info_size += SEC_FINGER_INFO_SZ;
	hx_s_touch_data->rawdata_size_normal -= SEC_FINGER_INFO_SZ;
#endif
...
#if defined(SEC_PALM_FUNC)
		g_target_report_data->maj = kzalloc(sizeof(int)*(hx_s_ic_data->max_pt), GFP_KERNEL);
		if (g_target_report_data->maj == NULL)
			goto mem_alloc_fail_sec_palm_maj;
		g_target_report_data->min = kzalloc(sizeof(int)*(hx_s_ic_data->max_pt), GFP_KERNEL);
		if (g_target_report_data->min == NULL)
			goto mem_alloc_fail_sec_palm_min;
		g_target_report_data->palm = kzalloc(sizeof(int)*(hx_s_ic_data->max_pt), GFP_KERNEL);
...
void himax_cable_detect_func(bool force_renew)
{
	...
	connect_status = lux_charger_mode_status;/* upmu_is_chr_det(); */
	ts = hx_s_ts;
...
/* SMWP */
	...
	if (hx_s_ic_data->stylus_func && ts_path != HX_REPORT_SMWP_EVENT)
		length -= STYLUS_INFO_SZ;
...
skip_stylus_operation:
	...
	hx_s_touch_data->coord_buf[base - 5];
#if defined(SEC_PALM_FUNC)
	palm = (hx_s_touch_data->state_info[0] >> 3 & 0x01);
#endif
	if (g_ts_dbg != 0)
...
#if defined(SEC_PALM_FUNC)
		maj = hx_s_touch_data->coord_buf[ts->coordInfoSize+(i*2)];
		min = hx_s_touch_data->coord_buf[ts->coordInfoSize+(i*2)+1];
#endif
		if (g_ts_dbg != 0)
...
int himax_chip_common_init(void)
{
	...
	if ((err = himax_parse_dt(ts, pdata)) < 0) {
...
extern unsigned int __attribute__ ((weak)) is_shut_down(void);
