...
//+P86801AA4,zengshangwen.lx,add,20250829,add to double click function
extern void __attribute__ ((weak)) set_tp_gesture_flag(bool flag);
//+P86801AA4,zengshangwen.lx,add,20250829,add to double click function
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/17,add tp earjack_mode
	if (lux_earphone_mode_status == 1) {
		hx_s_core_fp._set_earphone_prevent_enable(lux_earphone_mode_status, ts->suspended);
	}
//-P86801AA1,peiyuexiang.wt,modify,2023/07/17,add tp earjack_mode
...
static void hxtp_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
		 struct panel_event_notification *notification, void *client_data)
{
	...
	D("Notification type:%d, early_trigger:%d \n",
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger && ts != NULL && ts->dev != NULL) {
			I("early resume, TP_RESUME_THREAD\n");
			queue_delayed_work(ts->ts_int_workqueue, &ts->ts_int_work,
				msecs_to_jiffies(DELAY_TIME));
		}
		else
			D("resume notification post commit, nothing to do\n");
		break;
	case DRM_PANEL_EVENT_BLANK:
		if (notification->notif_data.early_trigger && ts != NULL && ts->dev != NULL) {
			I("early suspend, TP_SUSPEND\n");
#if defined(HX_CONTAINER_SPEED_UP)
			cancel_delayed_work(&ts->ts_int_work);
#endif
			himax_common_suspend(ts->dev);
		} else {
			D("suspend notification post commit, nothing to do\n");
		}
		break;
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		D("shashank:Received fps change old fps:%d new fps:%d\n",
				notification->notif_data.old_fps,
				notification->notif_data.new_fps);
		break;
	default:
		D("notification serviced :%d\n",
				notification->notif_type);
		break;
	}
}
...
static void himax_notifier_register(struct work_struct *work)
{
	...
	if (active_panel) {
		cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
					...
					&hxtp_ts_panel_notifier_callback, ts);
		if (!cookie)
			pr_err("Failed to register for panel events\n");
		I("registered for panel notifications panel: 0x%x\n", active_panel);
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	I("Enter %s\n", __func__);
    if (!panel_name_tp[0]) {
        I("panel_name_tp is missing.");
        return -EIO;
	} else
		I("panel_name_tp is %s.", panel_name_tp);
//+P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
	if ((strcmp(panel_name_tp,"lide_hx83102j_wt_dsi_vdo_90hz_hsd") != 0) &&
	(strcmp(panel_name_tp,"starry_hx83102j_wt_dsi_vdo_90hz_mdt") != 0) &&
	(strcmp(panel_name_tp,"starry_hx83102j_wt_dsi_vdo_90hz_mdt_agc") != 0)) {
		I("panel_name_tp Match failed\n");
		return -EIO;
	}
//-P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
...
module_param_string(dsi_display0, panel_name_tp, 512, 0600);
MODULE_PARM_DESC(dsi_display0,
	"himax_mmi.dsi_display0=<display node>: where <display node> is 'primary dsi display node name'");
