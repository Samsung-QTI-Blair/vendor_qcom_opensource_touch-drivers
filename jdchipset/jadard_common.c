...
extern void jadard_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
...
void jadard_ts_work(struct jadard_ts_data *ts)
{
    ...
    if ((ts->suspended) && (ts->pm_suspend)) {
        ret = wait_for_completion_timeout(
            &ts->pm_completion,
            msecs_to_jiffies(JD_TIMEOUT_COMERR_PM));
        if (!ret) {
            JD_E("Bus don't resume from pm(deep),timeout,skip irq");
            return;
        }
    }
#endif
...
static ssize_t jd_GESTURE_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    for (i = 0; i < size - 1 && i < JD_GEST_SUP_NUM; i++) {
        ...
        JD_I("%s:gesture_cust_en[%d] = %d\n", __func__, i, ts->gesture_cust_en[i]);
    }
...
static const struct proc_ops jadard_proc_fac_test_ops = {
    .proc_open    = fac_selftest_proc_open,
    .proc_read    = seq_read,
...
static void jadard_fb_register(struct work_struct *work)
{
    ...
    JD_I("%s: enter\n", __func__);
    //ts->fb_notif.notifier_call = jadard_fb_notifier_callback; not need in new qcom notifier
#ifdef JD_CONFIG_DRM
    if (ts->active_panel) {
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
                    PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, ts->active_panel,
                    &jadard_ts_panel_notifier_callback, ts);
        if (!cookie)
            pr_err("Failed to register for panel events\n");
        JD_I("registered for panel notifications panel: 0x%x\n", ts->active_panel);
