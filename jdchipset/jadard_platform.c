...
#ifdef JD_CONFIG_DRM
void jadard_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
		 struct panel_event_notification *notification, void *client_data)
{
	struct jadard_ts_data *ts = client_data;
	...
	JD_D("Notification type:%d, early_trigger:%d\n",
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger) {
            ...
            JD_I("early resume, TP_RESUME_THREAD\n");
            queue_delayed_work(ts->jadard_resume_wq, &ts->jadard_resume_work,
                msecs_to_jiffies(JD_RESUME_DELAY_TIME));
#else
            JD_D("early resume Skipped\n");
#endif
        } else {
#if defined(JD_RESUME_NOT_WAIT_FW)
            JD_D("normal resume, Skipped\n");
#else
            JD_I("normal resume, TP_RESUME\n");
	...
	case DRM_PANEL_EVENT_BLANK:
		if (notification->notif_data.early_trigger) {
            JD_I("early suspend, TP_SUSPEND\n");
            jadard_common_suspend(ts->dev);
		} else {
			JD_D("normal suspend, Skipped\n");
		}
		break;
	case DRM_PANEL_EVENT_BLANK_LP:
		JD_D("received lp event\n");
		break;
	case DRM_PANEL_EVENT_FPS_CHANGE:
		JD_D("shashank:Received fps change old fps:%d new fps:%d\n",
				notification->notif_data.old_fps,
				notification->notif_data.new_fps);
		break;
	default:
		JD_D("notification serviced :%d\n",
				notification->notif_type);
		break;
	}
}
                            ...
                            unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data &&
        ...
        ts != NULL &&
        ts->dev != NULL)  //zsw FB_EARLY_EVENT_BLANK -> FB_EVENT_BLANK
    {
        blank = evdata->data;
        ...
        case FB_BLANK_UNBLANK:
            if (FB_EVENT_BLANK == event) {  //zsw FB_EARLY_EVENT_BLANK -> FB_EVENT_BLANK
                JD_I("resume: event = %lu, Skipped\n", event);
...
int jadard_chip_common_probe(struct spi_device *spi)
{
    ...
    if (g_module_fp.head_support_chip == NULL) {
        JD_I("%s: Initial jadard_jd9366tp_probe\n", __func__);
...
void jadard_chip_common_remove(struct spi_device *spi)
{
...
#if defined(JD_PATCH_COMERR_PM)
static int jd_common_pm_suspend(struct device *dev)
{
	struct jadard_ts_data *ts = pjadard_ts_data;
	JD_I("system enters into pm_suspend");
	ts->pm_suspend = true;
	reinit_completion(&ts->pm_completion);
	return 0;
}

static int jd_common_pm_resume(struct device *dev)
{
	struct jadard_ts_data *ts = pjadard_ts_data;
	JD_I("system resumes from pm_suspend");
	ts->pm_suspend = false;
	complete(&ts->pm_completion);
	return 0;
}
static const struct dev_pm_ops jd_common_pm_ops = {
	.suspend = jd_common_pm_suspend,
	.resume  = jd_common_pm_resume,
};
#endif
...
static const struct spi_device_id jadard_id_table[] = {
