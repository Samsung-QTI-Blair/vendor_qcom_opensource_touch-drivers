...
static void module_jd9366tp_read_fw_ver(void)
{
    ...
    snprintf(hardinfo,HARDWARE_MAX_ITEM_LONGTH,"XINXIAN,JD9366,FW:0x%04X", pjadard_ic_data->fw_cid_ver);
...
#else
int jadard_jd9366tp_probe(void)
{
    struct jadard_support_chip *pChip = NULL;
    struct jadard_support_chip *ptr = NULL;
 ...
 int  jadard_jd9366tp_init(void)
{
    JD_I("%s\n", __func__);
    if (!panel_name_tp[0]) {
        JD_I("%s:panel_name_tp is missing.", __func__);
        return -1;
    }

    if (strcmp(panel_name_tp,"xinxian_jd9366ts_wt_dsi_vdo_90hz_csot") != 0){
        JD_I("panel_name_tp Match failed\n");
        return -1;
    }
...
MODULE_PARM_DESC(dsi_display0,
	"jadard_touch.dsi_display0=<display node>: where <display node> is 'primary dsi display node name'");
