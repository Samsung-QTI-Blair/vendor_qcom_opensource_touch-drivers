...
static int drm_check_dt(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data && ts_data->dev && ts_data->dev->of_node) {
        count = of_count_phandle_with_args(ts_data->dev->of_node, "panel", NULL);
        if (count <= 0) {
        ...
        for (i = 0; i < count; i++) {
            node = of_parse_phandle(ts_data->dev->of_node, "panel", i);
...
//+P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
    if (!panel_name_tp[0]) {
        FTS_ERROR("panel_name_tp name missing, exit!");
        return -ENOMEM;;
    } else {
        FTS_INFO("panel_name_tp is, %s", panel_name_tp);
    }

    if ((strcmp(panel_name_tp,"focaltech_ft8203_wt_dsi_vdo_90hz_apf_boe") != 0) &&
	(strcmp(panel_name_tp,"ft8203_wt_dsi_vdo_90hz_focaltech") != 0) &&
	(strcmp(panel_name_tp,"dsbj_ft8203_wt_dsi_vdo_90hz_inx") != 0)) {
		FTS_ERROR("lcm name Match failed!\n");
		return -ENOMEM;
    }
//-P86801AA1,wanwen2.wt,modify,2023/08/29,add lcm_name identify
...
// +P86801AA1 peiyuexiang.wt,modify,20230511,modify parse drm_panel
...
MODULE_PARM_DESC(dsi_display0,
	"focaltech_tp.dsi_display0=<display node>: where <display node> is 'primary dsi display node name'");
