...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
    case MODE_EARJACK:
        ret = fts_write_reg(FTS_REG_EARJACK_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_EARJACK switch to %d fail", m_val);
        }
        break;
//-P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
    default:
...
// +P86801AA1 peiyuexiang.wt,add,20230511,add touch sensitivity func
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    FTS_INFO("glove mode:%d", ts_data->glove_mode);
    return count;
}
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    FTS_INFO("cover mode:%d", ts_data->cover_mode);
    return count;
}
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
void fts_earjack_mode_callback(void)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;

    if (fts_data->earjack_plug_status == 1) {
        if (!ts_data->earjack_mode) {
//            FTS_DEBUG("enter earjack mode");
            ret = fts_ex_mode_switch(MODE_EARJACK, ENABLE);
            if (ret >= 0) {
                ts_data->earjack_mode = ENABLE;
            }
        }
    } else {
        if (ts_data->earjack_mode) {
//            FTS_DEBUG("exit earjack mode");
            ret = fts_ex_mode_switch(MODE_EARJACK, DISABLE);
            if (ret >= 0) {
                ts_data->earjack_mode = DISABLE;
            }
        }
    }

    FTS_INFO("earjack_mode:%d", ts_data->earjack_mode);
}
//-P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
