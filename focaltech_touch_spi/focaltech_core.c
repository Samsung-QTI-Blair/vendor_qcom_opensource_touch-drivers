...
extern void fts_earjack_mode_callback(void);
//-P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
...
//+P86801AA1,wanwen2.wt,modify,2023/08/29,judgment report SYN_REPORT
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_PROTOCOL_v2:
	    ...
	    events[i].p = touch_buf[FTS_TOUCH_OFF_MISC + base];
            if (events[i].p <= 0) events[i].p = 0x3F;
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
	if (ts_data->earjack_plug_status) {
		ts_data->earjack_plug_status = 0;
		ts_data->earjack_mode = 0;
		FTS_INFO("TP_LOG:%s:[tp earjack]EARJACK_PLUG_OUT", __func__);
	}
//-P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
...
// +P86801AA1 peiyuexiang.wt,add,20230531,Focaltech_ft8203 consumption issue
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
	if (lux_earphone_mode_status == 1) {
		earjack_notifier_call_chain_for_tp(EARJACK_PLUG_IN,NULL);
		FTS_INFO("TP_LOG:%s:[tp earjack]EARJACK_PLUG_IN", __func__);
	}
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	//FTS_DEBUG("Notification type:%d, early_trigger:%d", notification->notif_type, notification->notif_data.early_trigger);
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger) {
			...
			FTS_INFO("normal resume, TP_RESUME_THREAD");
			queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
		}
		break;
	case DRM_PANEL_EVENT_BLANK:
		if (notification->notif_data.early_trigger) {
			FTS_INFO("early suspend, TP_SUSPEND");
			cancel_work_sync(&fts_data->resume_work);
			fts_ts_suspend(ts_data->dev);
		} else {
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		//FTS_DEBUG("shashank:Received fps change old fps:%d new fps:%d", notification->notif_data.old_fps, notification->notif_data.new_fps);
		break;
	default:
		//FTS_DEBUG("notification serviced :%d", notification->notif_type);
		break;
	}
}
#endif //CONFIG_DRM
...
//+P86801AA1,peiyuexiang.wt,modify,2023/06/19,add charger_mode
static int ft8203_charger_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
...
//    FTS_FUNC_ENTER();
	...
	ts_data->notifier_charger.notifier_call = ft8203_charger_notifier_callback;
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
static int ft8203_earjack_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
...
static int fts_earjack_notifier_callback_init(struct fts_ts_data *ts_data)
{
    int ret = 0;
//    FTS_FUNC_ENTER();
	...
	ts_data->notifier_earjack.notifier_call = ft8203_earjack_notifier_callback;
	ret = earjack_register_notifier_chain_for_tp(&ts_data->notifier_earjack);
	if (ret < 0)
		FTS_ERROR("[earjack]Unable to register earjack_notifier: %d", ret);
...
static int fts_notifier_callback_init(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data && ts_data->dev && ts_data->dev->of_node)
        ...
        FTS_ERROR("%s: ts_data is NULL", __func__);

    ret = of_property_read_string(np, "focaltech,touch-type",
                &touch_type);
    if (ret)
        FTS_ERROR("%s: No touch type", __func__);
    if (strcmp(touch_type, "primary"))
        FTS_ERROR("Invalid touch type");

#if IS_ENABLED(CONFIG_DRM)
#if IS_ENABLED(CONFIG_DRM_PANEL)
    FTS_INFO("init notifier with panel_event_notifier_register");
    if (ts_data->active_panel) {
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
                    PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, ts_data->active_panel,
...
#if IS_ENABLED(CONFIG_DRM)
#if IS_ENABLED(CONFIG_DRM_PANEL)
	if (ts_data->active_panel && ts_data->notifier_cookie)
...
//+P86801AA1 peiyuexiang.wt,add,20230425,Focaltech_8203 openshort
#ifdef WT_OPEN_SHORT_PROC
#define CTP_PARENT_PROC_NAME			"touchscreen"
#define CTP_OPEN_PROC_NAME				"ctp_openshort_test"
#define  CTP_OPEN_SHORT_TEST_INI_NAME	"focaltech_ft8203_mp_boe.ini"
#define  CTP_OPEN_SHORT_TEST_INI_NAME2	"focaltech_ft8203_mp_dsbj.ini"
static int tp_openshort_result;
extern int fts_ft8203_open_short_test(char *ini_file_name);

static void fts_set_ini_name(char *cfgname)
{
    if (strstr(panel_name_tp,"focaltech_ft8203_wt_dsi_vdo_90hz_apf_boe")) {
        sprintf(cfgname, "%s", CTP_OPEN_SHORT_TEST_INI_NAME);
	}
    if (strstr(panel_name_tp,"ft8203_wt_dsi_vdo_90hz_focaltech")) {
        sprintf(cfgname, "%s", CTP_OPEN_SHORT_TEST_INI_NAME);
    }
    if (strstr(panel_name_tp,"dsbj_ft8203_wt_dsi_vdo_90hz_inx")) {
        sprintf(cfgname, "%s", CTP_OPEN_SHORT_TEST_INI_NAME2);
    }
}
...
static int openshort_test_proc_show(struct seq_file *m, void *v)
{
    ...
    tp_openshort_result = fts_ft8203_open_short_test(fwname);

    FTS_INFO("tp_openshort_result = %d", tp_openshort_result);
...
void fts_ft8203_create_ctp_proc(void)
{
    //----------------------------------------
    ...
    printk("openshort fts_ft8203_create_ctp_proc \n");
...
#ifdef WT_OPEN_SHORT_PROC
    ctp_open_proc = proc_create(CTP_OPEN_PROC_NAME, 0777, ctp_device_proc, &proc_ctp_openshort_test_fops);
...
// +P86801AA1 peiyuexiang.wt,add,20230425,Focaltech_ft8203 openshort
#ifdef WT_OPEN_SHORT_PROC
     fts_ft8203_create_ctp_proc();
#endif
// -P86801AA1 peiyuexiang.wt,add,20230425,Focaltech_ft8203 openshort
...
//+P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
    ret = fts_earjack_notifier_callback_init(ts_data);
    if (ret) {
        FTS_ERROR("init earjack notifier callback fail");
    }
//-P86801AA1,peiyuexiang.wt,modify,2023/07/06,add tp earjack_mode
