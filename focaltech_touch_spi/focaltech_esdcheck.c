...
*****************************************************************************/
#if LCD_ESD_PATCH
extern int __attribute__ ((weak)) get_focal_esdflag_status(void);
extern void __attribute__ ((weak)) set_focal_esdflag_status(int status);
static int tp_need_recovery; /* LCD reset cause Tp reset */
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    int ret = 0;
    u8 val = 0;
    static int focal_reset_flag = 0;

    focal_reset_flag = get_focal_esdflag_status();
//    FTS_DEBUG("check LCD ESD");
    if ( (tp_need_recovery == 1) && (focal_reset_flag == 0) ) {
        tp_need_recovery = 0;
    ...
    if (val == 0xAA) {
        /*
        * 1. Set flag focal_lcd_need_reset = 1;
        * 2. LCD driver need reset(recovery) LCD and set focal_lcd_need_reset to 0
        * 3. recover TP state
        */
        FTS_INFO("LCD ESD, need execute LCD reset");
	set_focal_esdflag_status(1);
        tp_need_recovery = 1;
    }
